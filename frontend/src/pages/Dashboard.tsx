...
const getStatusConfig = (status: string) => {
  switch (status) {
    case "submitted":
      return { label: "Submitted", class: "status-submitted", icon: FileText };
    case "under-verification":
      return { label: "Under Verification", class: "status-verification", icon: Clock };
    case "forwarded":
      return { label: "Forwarded to Sponsor", class: "status-forwarded", icon: TrendingUp };
    case "approved":
      return { label: "Approved", class: "status-approved", icon: CheckCircle2 };
    case "rejected":
      return { label: "Rejected", class: "status-rejected", icon: XCircle };
    default:
      return { label: status, class: "status-submitted", icon: FileText };
  }
};
...
const Dashboard = () => {
  ...
  const { data: notificationsResponse, isLoading: isLoadingNotifs } = useQuery({
  ...
  const stats = {
    ...
    pending: applications.filter((a: any) => ["submitted", "under-verification", "forwarded"].includes(a.status)).length,
    rejected: applications.filter((a: any) => a.status === "rejected").length,
    awardedAmount: applications
      .filter((a: any) => a.status === "approved")
      .reduce((sum: number, a: any) => sum + (a.scholarship?.amount || 0), 0),
  };
                    ...
                    {isLoadingApps ? (
                      ...
                      <>
                        ...
                        <TabsContent value="pending" className="space-y-4 mt-0">
                          {applications
                            .filter((a: any) => ["submitted", "under-verification", "forwarded"].includes(a.status))
                            .map((app: any) => {
                              const statusConfig = getStatusConfig(app.status);
                              const StatusIcon = statusConfig.icon;
                              const scholarshipTitle = app.scholarship?.title || "Unknown Scholarship";
                  ...
                  <div className="space-y-3">
                    {isLoadingNotifs ? (
                      ...
                      notifications.map((notif: any) => (
                        <div
                          key={notif._id}
                          className={`p-3 rounded-lg border ${notif.read ? "border-border" : "border-primary/20 bg-primary/5"
                            }`}
                        >
