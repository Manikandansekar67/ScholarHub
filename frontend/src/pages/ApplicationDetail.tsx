...
const getStatusConfig = (status: string) => {
    switch (status) {
        case "submitted":
            return { label: "Submitted", class: "status-submitted", icon: FileText, description: "Your application has been received and is awaiting initial review." };
        case "under-verification":
            return { label: "Under Verification", class: "status-verification", icon: Clock, description: "Your documents are currently being verified by the institution." };
        case "forwarded":
            return { label: "Forwarded to Sponsor", class: "status-forwarded", icon: TrendingUp, description: "Your application has been verified and forwarded to the sponsor for final decision." };
        case "approved":
            return { label: "Approved", class: "status-approved", icon: CheckCircle2, description: "Congratulations! Your application has been approved." };
        case "rejected":
            return { label: "Rejected", class: "status-rejected", icon: XCircle, description: "We regret to inform you that your application was not selected." };
        default:
            return { label: status, class: "status-submitted", icon: FileText, description: "Status update pending." };
    }
};
                ...
                <div className="container py-8">
                    <div className="grid lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2 space-y-8">
                            ...
                            {/* Status Details */}
                            <Card className={`border-l-4 ${application.status === 'approved' ? 'border-l-success' :
                                    application.status === 'rejected' ? 'border-l-destructive' :
                                        'border-l-primary'
                                }`}>
                                ...
                                <CardContent className="space-y-4">
                                    <div>
                                        ...
                                        <p className="text-2xl font-bold">
                                            {new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 }).format(scholarship?.amount || 0)}
                                        </p>
                                    </div>
