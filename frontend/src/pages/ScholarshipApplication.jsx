...
const ScholarshipApplication = () => {
  ...
  const handleFileUpload = async (e, requirementIndex) => {
  ...
  const onSubmit = data => {
    ...
    const missingDocs = requiredDocs.some((_, index) => !uploadedDocuments[`doc_${index}`]);
    if (missingDocs) {
  ...
  return <div className="min-h-screen flex flex-col">
            ...
            <main className="flex-1 py-8">
                <div className="container max-w-3xl">
                    ...
                    {generalError && <Alert variant="destructive" className="mb-6">
                            ...
                            <CardContent>
                                <div className="grid md:grid-cols-2 gap-4">
                                    ...
                                    <div className="md:col-span-2">
                                        ...
                                        <Textarea id="coverLetter" placeholder="Why are you a good candidate for this scholarship?" className="mt-2 min-h-[150px]" {...register("coverLetter", {
                    required: "Cover letter is required"
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-6">
                                {requiredDocuments.length === 0 ? <p className="text-muted-foreground text-sm italic">No specific documents required for this application.</p> : requiredDocuments.map((docName, index) => {
                const isUploaded = !!uploadedDocuments[`doc_${index}`];
                const isUploading = uploadingFiles[`doc_${index}`];
                return <div key={index} className="border border-border rounded-lg p-4">
                                                <div className="flex items-center justify-between mb-3">
                                                    <Label className="text-base font-medium">
                                                        {docName} <span className="text-destructive">*</span>
                                                    </Label>
                                                    {isUploaded && <Badge variant="secondary" className="bg-success/10 text-success hover:bg-success/20 border-0">
                                                            <CheckCircle2 className="h-3 w-3 mr-1" />
                                                            Uploaded
                                                        </Badge>}
                                                </div>

                                                {isUploaded ? <div className="flex items-center justify-between bg-muted/50 p-3 rounded-md">
                                                        ...
                                                        <Button type="button" variant="ghost" size="sm" onClick={() => removeDocument(index)} className="h-8 w-8 p-0 text-muted-foreground hover:text-destructive">
                                                            <X className="h-4 w-4" />
                                                        </Button>
                                                    </div> : <div className="relative">
                                                        <input type="file" id={`file-${index}`} className="hidden" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" onChange={e => handleFileUpload(e, index)} disabled={isUploading} />
                                                        <Label htmlFor={`file-${index}`} className={`flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg cursor-pointer hover:bg-muted/50 transition-colors ${isUploading ? "opacity-50 cursor-not-allowed" : "border-muted-foreground/25"}`}>
                        ...
                        <div className="flex flex-col gap-4 pt-4">
                            <Button type="submit" size="lg" disabled={submitMutation.isPending} className="w-full sm:w-auto self-end">
