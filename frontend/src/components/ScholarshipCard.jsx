...
const ScholarshipCard = ({
  ...
  const getStatusBadge = () => {
    if (scholarship.status === "closing-soon") {
      return <Badge variant="outline" className="status-warning border">
          <Clock className="h-3 w-3 mr-1" />
          Closing Soon
        </Badge>;
    }
    if (scholarship.status === "closed") {
      return <Badge variant="outline" className="status-rejected border">
          Closed
        </Badge>;
    }
    return <Badge variant="outline" className="status-approved border">
        Open
      </Badge>;
  };
  return <Card className="card-elevated h-full flex flex-col">
